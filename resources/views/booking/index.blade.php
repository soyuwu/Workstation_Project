                ...
                @foreach ($services as $slug => $service)
                    <article class="group relative overflow-hidden rounded-3xl bg-white shadow-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-[var(--shadow-card-hover)]">
                        ...
                        <div class="p-8">
                            ...
                            <p class="mb-6 text-sm leading-relaxed text-slate-500 line-clamp-2">{{ $service['desc'] }}</p>
                            ...
                            <div class="mb-6 flex items-center gap-2">
                                <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
                                    <span class="material-symbols-outlined text-[14px]">
                                        {{ $service['type'] === 'monthly' ? 'calendar_month' : 'schedule' }}
                                    </span>
                                    {{ $service['type'] === 'monthly' ? 'Thuê theo tháng' : 'Thuê theo giờ/ngày' }}
                                </span>
                            </div>

                            <a href="{{ route($service['type'] === 'monthly' ? 'booking.monthly' : 'booking.hourly', $slug) }}" 
