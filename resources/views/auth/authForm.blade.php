...
<body
    class="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(219,234,254,0.95),_rgba(248,250,252,1)_68%)] px-4 py-6 sm:px-6 lg:px-10">
    <main class="mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl items-center justify-center">
        <section id="container" data-auth-container
            class="auth-card {{ request()->is('register') ? 'is-register' : '' }}">
            <div class="absolute left-1/2 top-6 z-20 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 space-y-3">
                ...
                @if (session('warning'))
                    <div
                        class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 shadow-sm">
                        {{ session('warning') }}
                    </div>
                @endif
            ...
            <div class="auth-mobile-toggle absolute left-6 right-6 top-6 z-20 rounded-2xl bg-slate-100 p-1">
                <button type="button" data-auth-target="login" data-auth-toggle
                    class="flex-1 rounded-xl px-4 py-2 text-sm font-semibold text-slate-600 transition-all duration-200">
                    Đăng nhập
                </button>
                <button type="button" data-auth-target="register" data-auth-toggle
                    class="flex-1 rounded-xl px-4 py-2 text-sm font-semibold text-slate-600 transition-all duration-200">
                    Đăng ký
                </button>
            </div>
            ...
            <div class="auth-form-panel auth-form-panel--signin px-8 py-12 sm:px-10 lg:px-14">
                <div class="mx-auto flex w-full max-w-md flex-1 flex-col justify-center">
                    <a href="{{ url('/') }}"
                        class="mb-8 inline-flex items-center gap-2 text-sm font-semibold text-primary">
                        <span class="text-lg">←</span>
                        Quay lại trang chủ
                    </a>

                    <span
                        class="mb-4 inline-flex w-fit rounded-full bg-primary-light px-4 py-2 text-sm font-semibold text-primary">
                        Welcome back
                    </span>
                    ...
                    <div class="mb-8 grid grid-cols-3 gap-3">
                        <button type="button"
                            class="rounded-2xl border border-slate-200 px-4 py-3 text-slate-500 transition-colors hover:border-primary hover:text-primary">
                    ...
                    <form method="POST" action="{{ url('/logIn') }}" class="space-y-4">
                        ...
                        <label class="block">
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required
                                class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition-all duration-200 focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/10">
                        </label>
                        ...
                        <button type="submit"
                            class="inline-flex w-full items-center justify-center rounded-2xl bg-primary px-6 py-3.5 text-sm font-semibold text-white transition-all duration-300 hover:bg-primary-dark active:scale-[0.99]">
                            Đăng nhập
                        </button>
                    </form>
                </div>
            ...
            <div class="auth-form-panel auth-form-panel--signup px-8 py-12 sm:px-10 lg:px-14">
                <div class="mx-auto flex w-full max-w-md flex-1 flex-col justify-center">
                    ...
                    <span
                        class="mb-4 inline-flex w-fit rounded-full bg-accent-light px-4 py-2 text-sm font-semibold text-accent">
                        New member
                    </span>
            ...
            <div class="auth-overlay">
                <div class="auth-overlay-track"
                    ...
                    <div class="auth-overlay-panel text-white">
                        <span
                            class="mb-4 rounded-full border border-white/20 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em]">
                            WorkStation
                        </span>
                        <h2 class="mb-4 font-headline text-4xl font-bold">Hello, Friend!</h2>
                        <p class="mb-8 max-w-sm text-base text-blue-100">
                            Tạo tài khoản để khám phá không gian làm việc linh hoạt, quản lý booking và nhận các ưu đãi
                            mới nhất từ WorkStation.
                        </p>
                        <button type="button" data-auth-target="register"
                            class="rounded-2xl border border-white/30 px-8 py-3 text-sm font-semibold text-white transition-all duration-300 hover:bg-white/10">
                            Đăng ký ngay
                        </button>
                    </div>
                    ...
                    <div class="auth-overlay-panel text-white">
                        ...
                        <p class="mb-8 max-w-sm text-base text-blue-100">
                            Đăng nhập để tiếp tục đặt lịch nhanh hơn, theo dõi lịch sử sử dụng và làm việc cùng cộng
                            đồng sáng tạo của bạn.
                        </p>
                        <button type="button" data-auth-target="login"
                            class="rounded-2xl border border-white/30 px-8 py-3 text-sm font-semibold text-white transition-all duration-300 hover:bg-white/10">
                            Đăng nhập
                        </button>
                    </div>
