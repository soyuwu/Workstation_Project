        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-10 text-center">
            @if($booking->status === 'cancelled')
                <div class="w-20 h-20 bg-rose-100 rounded-full flex items-center justify-center mx-auto mb-6">
                    <span class="material-symbols-outlined text-4xl text-rose-500">cancel</span>
                </div>
                <h1 class="text-2xl font-bold text-slate-800 mb-2">Đơn đã hủy</h1>
            @else
            ...
            <p class="text-slate-500 mb-8 leading-relaxed">
                {{ $message ?? 'Yêu cầu của bạn đã được ghi nhận. Chúng tôi sẽ xử lý trong thời gian sớm nhất.' }}
            </p>
                ...
                <div class="flex justify-between items-center">
                    ...
                    @if($booking->status === 'cancelled')
                        <span class="font-medium text-rose-600 flex items-center gap-1">
                            <span class="material-symbols-outlined text-sm">cancel</span> Đã hủy
                        </span>
            ...
            <p class="text-xs text-slate-400 mt-4">
                ...
                <span id="countdown-num" class="font-bold text-primary">10</span> giây...
            </p>
        </div>
    </div>
</div>
